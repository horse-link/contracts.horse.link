...
def main():
    ...
    # array of propositions that have already been sent to the Oracle contract
    processed_items = []
      ...
      print(f"Found {len(runners)} scratched runners")
        ...
        for runner in runners:
            ...
            if proposition_id not in processed_items:
                ...
                print(
                    f"Sending proposition {proposition_id} to Oracle contract")
                odds = runner["odds"] * 1000000
                totalOdds = int(runner["totalOdds"])
                signature = runner["signature"]
                print(
                    f"set_scratch(oracle, {proposition_id}, {market_id}, {odds}, {totalOdds}, {signature}")
                set_scratch(oracle, proposition_id, market_id,
                            odds, totalOdds, signature)
                print("Sent.")
