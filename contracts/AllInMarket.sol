...
contract AllInMarket is IMarket, Ownable, ERC721 {
	...
	function multiBack(
		...
		for (uint8 i; i < backArray.length; i++) {
			...
			indexes[i] = _getPayoutAndBack(data);
		}

		return indexes;
	}

	function back(Back calldata backData) external returns (uint256) {
		return _getPayoutAndBack(backData);
	}

	function _getPayoutAndBack(
		Back calldata backData
	)
		internal
		validBackSignature(backData)
		validBackDate(backData)
		returns (uint256)
	{
	...
	function settleMarket(bytes16 marketId) external {
		...
		for (uint64 i = 0; i < count; i++) {
			...
			if (bet.settled == false) {
				_settle(index, bet.marketId, bet.propositionId);
			}
		}
	}
	...
	function _settle(
		...
		uint8 result = IOracle(_oracle).checkResult(marketId, propositionId);
	...
	modifier validBackSignature(Back memory backData) {
	...
	modifier validBackDate(Back memory backData) {
		require(
