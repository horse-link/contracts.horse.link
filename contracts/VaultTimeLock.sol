...
contract Vault is ERC4626Metadata, Ownable {
    ...
    // Override _mint function to require owner to be receiver
    // Set lock time on owner to block transfers until after lock period

function _mint(address account, uint256 amount) internal override {
        require(account == owner(), "Vault: Only owner can mint");
        super._mint(account, amount);
        lockedTime[account] = block.timestamp + lockDuration;
    }
