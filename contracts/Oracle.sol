...
contract Oracle is Ownable {
    ...
    mapping(bytes32 => mapping(uint8 => mapping(uint8 => mapping(uint8 => mapping(uint8 => Result)))))
        public results;
    ...
    function addResult(
        ...
        require(
            BokkyPooBahsDateTimeLibrary.timestampFromDate(year, month, day) <
                block.timestamp,
            "Cannot add results of future events"
        );
        require(
            results[mnemonic][year][month][day][race].timestamp == 0,
            "Results have already added"
        );
        require(_results.length <= 4, "Too many results");

        results[mnemonic][year][month][day][race] = Result(
            _results,
            block.timestamp
        );
        count++;
